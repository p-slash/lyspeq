...
namespace conv {
    void convertLambdaToOnePlusRedshift(double *lambda, int size) {
        std::for_each(lambda, lambda + size, [](double &ld) {
            ld = ld / LYA_REST;
        });
    }
}
    ...
    public:
        ...
        */
        double _nu_relative_density(
                z1, A=0.3173, nu_y0=357.91212097, p=1.83, invp=0.54644808743,
                nmassless=2, B=0.23058962986246165
        ) {
            return B * (nmassless + pow(1 + pow(A * nu_y0 / z1, p), invp));
        }

        /* in km/s/Mpc */
        double _calcHubble(z1) {
            double z3 = z1 * z1 * z1;
            double z4 = z3 * z1;
        ...
        double _calcComovingDist(z1) {

        }

        /* in km/s/Mpc */
        double getHubble(z1) {
            hubble_z->evaluate(z1);
        }

        double getComovingDist(z1) {
            interp_comov_dist->evaluate(z1);
        }
    }
}
