...
double FourierIntegrator::evaluateAToInfty(double a, double epsabs)
{
    ...
    double result, error;
    int status = gsl_integration_qawf(&F, a, epsabs, MY_WORKSPACE_SIZE, w, cycle_w, t, &result, &error);
...
double FourierIntegrator::evaluate(double a, double b, double omega, double epsabs, double epsrel)
{
    ...
    status = gsl_integration_qawo(&F, a, epsabs, epsrel, MY_WORKSPACE_SIZE, w, t, &result, &error);
