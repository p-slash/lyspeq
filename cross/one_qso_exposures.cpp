...
namespace xeutils {
    ...
    bool skipCombo(const Exposure *expo1, const Exposure *expo2) {
        ...
        skip |= (
            xeutils::_calculateOverlapRatio(expo1->qFile.get(), expo2->qFile.get())
...
void OneQsoExposures::setAllocPowerSpMemory() {
    ...
    for (const auto &[expo1, expo2] : exposure_combos) {
        xeutils::_setInternalVariablesForTwoExposures(expo1, expo2);

        istart = std::min(istart, xeutils::fisher_index_start);
        fisher_index_end = std::max(
            fisher_index_end,
            xeutils::fisher_index_start + xeutils::N_Q_MATRICES);
    }
...
void OneQsoExposures::xQmlEstimate() {
    xeutils::_vmatrix = glmemory::temp_matrices[0].get();
    xeutils::_zmatrix = glmemory::temp_matrices[1].get();
    ...
    for (const auto &[expo1, expo2] : exposure_combos) {
        ...
        #ifdef DEBUG
        assert(!xeutils::skipCombo(expo1, expo2));
        #endif

        // Contruct VZ matrices
        xeutils::_setVZMatrix();
        xeutils::_setStoredIkzQiVector();
        ...
        for (auto &[i_kz, qi, qwi] : xeutils::stored_ikz_qi_qwi) {
            dk0[i_kz + diff_idx] += mxhelp::my_cblas_dgemvdot(
                expo1->getWeightedData(), xeutils::N1,
                expo2->getWeightedData(), xeutils::N2,
                qi, glmemory::temp_vector.get());

            cblas_dgemm(
                CblasRowMajor, CblasNoTrans, CblasNoTrans,
                xeutils::N1, xeutils::N2, xeutils::N1, 1.,
                expo1->getInverseCov(), xeutils::N1,
                qi, xeutils::N2, 0,
                glmemory::temp_matrices[0].get(), xeutils::N2);

            cblas_dgemm(
                CblasRowMajor, CblasNoTrans, CblasNoTrans,
                xeutils::N1, xeutils::N2, xeutils::N2, 1.,
                glmemory::temp_matrices[0].get(), xeutils::N2,
                expo2->getInverseCov(), xeutils::N2, 0.,
                qwi, xeutils::N2);
        }
        ...
        if (!specifics::TURN_OFF_SFID) {
            ...
            for (const auto &[i_kz, qi, qwi] : xeutils::stored_ikz_qi_qwi)
                tk0[i_kz + diff_idx] += cblas_ddot(
                    xeutils::CROSS_SIZE_2, qwi, 1, glmemory::stored_sfid.get(), 1);
        }
        ...
        #pragma omp parallel for schedule(static, 1)
        for (   auto iqt = xeutils::stored_ikz_qi_qwi.cbegin();
                iqt != xeutils::stored_ikz_qi_qwi.cend(); ++iqt
        ) {
            int i_kz = 0, j_kz = 0;
            double *qwi = nullptr, *qjT = nullptr;
            std::tie(i_kz, std::ignore, qwi) = *iqt;

            for (auto jqt = iqt; jqt != xeutils::stored_ikz_qi_qwi.cend(); ++jqt) {
                ...
                fisher_matrix[j_kz + ndim * i_kz + diff_idx] += 
                    cblas_ddot(xeutils::CROSS_SIZE_2, qwi, 1, qjT, 1);
            }
        }
